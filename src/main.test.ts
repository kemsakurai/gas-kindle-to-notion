...
describe('Kindle Highlight Parsing', () => {
  ...
  test('extracts highlights with noteHeading and noteText pattern', () => {
    ...
    const highlights = [];
    for (const match of html.matchAll(noteRegex)) {
      highlights.push({
  ...
  test('extracts highlight colors', () => {
    const heading =
      '<span class="highlight_yellow">Chapter 1</span>, Location 123';
  ...
  test('strips HTML tags from text', () => {
    ...
    expect(stripHtml('<span style="color:red">Red text</span>')).toBe(
      'Red text',
    );
  });
  // Test section extraction
