 ...
 */
function extractHighlightsUsingMultiplePatterns(
  htmlContent: string,
 ...
 */
function extractHighlightsFallbackMethod(
  htmlContent: string,
  highlights: KindleHighlight[],
): void {
  try {
    ...
    for (const noteMatch of htmlContent.matchAll(noteRegex)) {
    ...
    if (highlights.length === 0) {
      ...
      for (const divMatch of htmlContent.matchAll(divRegex)) {
    ...
    logMessage(
      `フォールバックでのハイライト抽出エラー: ${error.message}`,
      true,
    );
  }
}
 ...
 */
function extractHighlightsFromSection(
  ...
  try {
    ...
    const extractionMethods = [
      ...
      () => {
        ...
        for (let i = 1; i < parts.length; i++) {
          ...
          if (headingParts.length > 1) {
            ...
            // noteTextを探す
            const textMatch = headingParts
              .slice(1)
              .join('</div>')
              .match(/<div class="noteText">(.*?)<\/div>/s);
            if (textMatch?.[1]) {
      ...
      () => {
        const noteRegex =
          /<div class="noteHeading">(.*?)<\/div>\s*<div class="noteText">(.*?)<\/div>/gs;
        for (const noteMatch of sectionContent.matchAll(noteRegex)) {
          const heading = noteMatch[1].trim();
          const text = noteMatch[2].trim();
